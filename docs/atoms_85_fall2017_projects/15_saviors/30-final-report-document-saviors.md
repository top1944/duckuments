...
In general as we are dealing with many color filters, we are quite dependent on the Anti Instagram module and collaborated closely with them together. As their continuous anti_instagram node and especially transformation computation consumes a considerable amount of computing power (80% at an transformation computation interval of 0.1Hz) we are currently considering an init procedure. This means to basically wait until one good transformation has been calculated and then updating this one very sparsely. Furthermore the image transformer node which is publishing the fully corrected images is currently operating at 4Hz and consumes about 75% computing power and introduces a delay of about 0.2seconds. So this interface has to be made more efficient, especially the 80% CPU load of calculating the trafo online should be saved.
Our algorithm on its own, needs an additional 0.3s to compute the obstacles and also needs about 75% CPU. So without adding an efficient init procedure, the full computational power of the Rasberry Pi is exploited if the two algorithms described above run together with the lane-following demo and our node is effectively only running at 1.5Hz. In the next weeks we have to come up with a more efficient implementation so get our algorithm back up to about 2.5-3Hz. (If setting the update interval of the continouus mode e.g. to 120s we arrive at this goal :)) 
Another Prerequisite for a good functioning of our algorithm is a good extrinsic calibration. 

### Eval Obstacle Detection {#saviors-eval-obst-detect}

In general, our algorithms are invariant to the current orientation of the duckiebot. So when looking at static situations and placing all different types of obstacles in front of our duckiebot in all different orientations. We have got a false positive percentage of below 1% and we label all of the obstacles correctly (only if we take the absolute smallest duckies and place them in front of the vehicle, so very very close (below 4 centimeters), without approaching them we cannot detect them).

So whenever we come to a stop, we will for sure evaluate the situation correctly.
This already implies that most of the misclassification errors are due to the effect of motion blur, assuming a stable color transformation provided by the anti instagram module. E.g. in LABEL two line segments in the background “blurred” together for two consecutive frames resulting in being labelled as an obstacle.  
