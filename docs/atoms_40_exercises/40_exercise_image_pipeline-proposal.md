...
where `![map file basename]` is the basename of the file without the extension. In order to do this exercise, you will have to fill in the details of the function `ground2pixel`
...
To load a map file, use the function `load_map` provided in the `Augmenter` class:

	map = load_map(map_filename)

### Reading the calibration data for a robot

To load the _intrinsic_ calibration parameters, use the function `load_camera_intrinsics` provided in the  `Augmenter` class: 

	intrinsics = load_camera_intrinsics(robot_name)

To load the _extrinsic_ calibration parameters (i.e. ground projection), use the function `load_homography` provided in the `Augmenter` class:
...
To remove the distortion from an image, use the function `rectify` provided in the `Augmenter` class:

	image = rectify(image)

### Drawing primitives

To draw all of the line segments specified in a map file, use the function `draw` provided in the `Augmenter` class:
